...
fn run_tray_app() -> Result<()> {
    ...
    let worker = worker::OcrWorker::spawn(models, platform::Waker::for_current_thread());
    ...
    let tray = tray::Tray::new(HOTKEY_LABEL)?;
    log::info!("ready: press {HOTKEY_LABEL} or click the tray icon");

    while platform::pump_message() {
        let mut capture = false;
        while let Ok(event) = GlobalHotKeyEvent::receiver().try_recv() {
            capture |= event.id == hotkey.id() && event.state == HotKeyState::Pressed;
        }
        match tray.poll() {
            Some(tray::TrayAction::Quit) => break,
            Some(tray::TrayAction::Capture) => capture = true,
            None => {}
        }
...
fn models_dir() -> Result<PathBuf> {
    ...
    exe_dir
        ...
        .chain(cfg!(debug_assertions).then(|| PathBuf::from(env!("CARGO_MANIFEST_DIR"))))
        .map(|d| d.join("models"))
        .find(|d| d.join("det").is_dir())
